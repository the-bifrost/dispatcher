...
async def initialize_db(db_path: Path):
    ...
    try:
        schema_sql = files("dispatcher.core.sql").joinpath("schema.sql").read_text(encoding="utf-8")
...
async def start(config_dir: Path, data_dir: Path, settings: Settings):
    _LOGGER.info("Inicializando a Bifrost")

    db_path = data_dir / settings.registry_db_path
